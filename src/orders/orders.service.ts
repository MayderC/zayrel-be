...
export class OrdersService {
  ...
  async create(createOrderDto: CreateOrderDto, authenticatedUser?: any) {
    // STORE_CHECKOUT_ENABLED: set to false to disable purchases
    const STORE_CHECKOUT_ENABLED = false;
    if (!STORE_CHECKOUT_ENABLED) {
      throw new ServiceUnavailableException('La tienda está cerrada temporalmente. Volvé pronto.');
    }

    // SECURITY: Log incoming data for debugging
    console.log(
      '[OrdersService.create] AuthenticatedUser:',
    ...
    if (authenticatedUser) {
      ...
      console.warn(
        `[SECURITY] Guest order attempted with user ID (${createOrderDto.user}). Ignoring.`,
      );
    }
    ...
    if (search && search.trim()) {
      ...
      if (/^[a-fA-F0-9]+$/.test(searchTerm) && searchTerm.length >= 3) {
        ...
        if (searchTerm.length === 24) {
          ...
          const allIds = await this.orderModel
            .find(status && status !== 'todas' ? { status } : {}, { _id: 1 })
            .lean()
            .exec();
  ...
  async findMyOrders(userId: string, userEmail?: string) {
    ...
    if (userObjectId && userEmail) {
      query.$or = [{ user: userObjectId }, { 'guestInfo.email': userEmail }];
  ...
  async updatePaymentProof(id: string, proofDto: UpdatePaymentProofDto) {
    ...
    if (proofDto.reason !== undefined) {
      ...
      order.paymentProof.reason =
        proofDto.reason || (proofDto.status === 'verified' ? undefined : currentProof.reason);
    }
  ...
  async updateStatus(id: string, status: string) {
    ...
    if (status !== previousStatus) {
      ...
      if (status === 'en_produccion') {
        ...
        // Award VTO tokens
        if (order.user) {
          await this.userModel.findByIdAndUpdate(order.user._id || order.user, {
            $inc: { vtoTokens: 5 },
          });
        }
      }
