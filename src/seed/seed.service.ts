...
export class SeedService implements OnApplicationBootstrap {
    ...
    private async seedDevUser() {
        ...
        if (!email || !password) {
            this.logger.warn('DEV_MODE=true pero DEV_USER_EMAIL o DEV_USER_PASSWORD no definidos. Saltando el seeding de dev user.');
            return;
        }
    ...
    private async upsertUser(opts: {
        ...
        try {
            const existingUser = await this.userModel.findOne({ email: email.toLowerCase() }).select('+password');
            ...
            if (existingUser) {
                ...
                this.logger.log(`${label} user (${email}) ya existía — contraseña y datos reseteados desde .env.`);
                return;
            }
