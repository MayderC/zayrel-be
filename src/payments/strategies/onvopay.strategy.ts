...
export class OnvopayStrategy implements PaymentGateway {
  ...
  async initiatePayment(
    ...
    this.logger.log(
      `Initiating Onvopay payment for Order ${orderId}, Amount: ${amount} ${currency}, Shipping: ${shippingCost}, Items: ${orderItems?.length || 0}`,
    );
  ...
  async verifyWebhook(payload: any, signature: string): Promise<boolean> {
    ...
    if (!webhookSecret) {
      this.logger.warn(
        'ONVOPAY_WEBHOOK_SECRET not configured - accepting all webhooks (development mode)',
      );
      return true;
    }
