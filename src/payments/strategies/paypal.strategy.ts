...
export class PaypalStrategy implements PaymentGateway {
  ...
  async initiatePayment(
    ...
    if (currency === 'CRC') {
      ...
      this.logger.log(
        `Converting CRC to USD: ${amount} CRC -> ${finalAmount.toFixed(2)} USD (rate: ${exchangeRate})`,
      );
    }
  ...
  async verifyWebhook(payload: any, signature: string): Promise<boolean> {
    ...
    this.logger.log(
      'PayPal Webhook received - skipping signature verification for now (Not Implemented)',
    );
    return true;
  }
}
