...
export class PaymentService {
  ...
  async initiatePayment(userId: string, dto: InitiatePaymentDto): Promise<PaymentResponse> {
    ...
    this.logger.debug(
      `Order data - user: ${JSON.stringify(orderData.user)}, guestInfo: ${JSON.stringify(orderData.guestInfo)}, shippingAddress: ${JSON.stringify(orderData.shippingAddress)}`,
    );
  ...
  private async handlePayPalWebhook(payload: any) {
    try {
      ...
      if (eventType === 'PAYMENT.CAPTURE.COMPLETED') {
        ...
        this.logger.log(
          `Order ${orderId} marked as PAID via PayPal. Capture ID: ${captureId}, Amount: ${amount} ${currency}`,
        );
    ...
    try {
      ...
      if (isSuccess) {
        ...
        if (onvopayId) {
          ...
          this.logger.log(
            `Order ${orderId} marked as paid via ${gateway} webhook. Onvopay ID: ${onvopayId}`,
          );
        } else {
          this.logger.log(
            `Order ${orderId} marked as paid via ${gateway} webhook (no Onvopay ID found)`,
          );
        }
