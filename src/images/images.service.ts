...
export class ImagesService {
  ...
  async setMainImage(productId: string, imageId: string): Promise<void> {
    ...
    const imageIndex = currentImages.findIndex((id) => id.toString() === imageId);
    if (imageIndex === -1) {
      throw new BadRequestException('Image not found in product');
    }

    // Move to front
    const newOrder = [imageObjectId, ...currentImages.filter((id) => id.toString() !== imageId)];
  ...
  async getProductImages(productId: string): Promise<ImageDocument[]> {
    const product = await this.productModel.findById(productId).populate('images');
   ...
   */
  private uploadToCloudinary(file: UploadedFile, folder: string): Promise<UploadApiResponse> {
